...
wait_and_retry() {
    ...
    certbot -v --non-interactive install -d "$DOMAIN" --cert-name "$DOMAIN" --nginx --no-redirect || exit 1
}
...
certbot -v --non-interactive install -d "$DOMAIN" --cert-name "$DOMAIN" --nginx --no-redirect || wait_and_retry
